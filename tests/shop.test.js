...
it('GET /:id - Retrieve a shop', async () => {
	const shopId = '66b346534fe43d77dbcfd376'
	...
	expect(res.body).toEqual({
		...
		shop: {
			...
			clickCollect: expect.toBeOneOf({
				instructions: expect.any(Array),
				openingHours: [
					{
					_id: expect.any(Array),
					periods: [
						{
						_id: expect.any(Array),
						closingTime: expect.any(Array),
						openingTime: expect.any(Array)
						}
					],
...
it('POST - createNewShop', async () => {
	const testKey = await getTestingKey()

	const res = await request(app).post(`/shops/?__clerk_testing_token=${testKey}&__clerk_testing_user_uuid=${testUUID}`).send({
